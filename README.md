...
|---|---|---|---|
| `baseline_cot` | - | Standard chain-of-thought reasoning | "Answer the question by thinking step by step. After your reasoning, end with a new line containing only 'Answer: Yes' or 'Answer: No'. Question: {question}" |
| `enumeratio` | Enumeratio | Explicitly numbered reasoning steps (1. 2. 3.) | "Answer the question using **numbered reasoning steps.** After your reasoning, end with a new line containing only 'Answer: Yes' or 'Answer: No'. Question: {question}" |
| `parison` | Parison | Parallel clause structure with repeated grammatical form across reasoning steps | "Answer the question using **parallel sentence structures** where possible, where each reasoning step follows the same grammatical form **(e.g. *'X is true because... Y is true because...'*)**. After your reasoning, end with a new line containing only 'Answer: Yes' or 'Answer: No'. Question: {question}" |
...
- **Figures rarely occur in isolation.** Enumeratio, parison, and chiasmus were operationalised as clean, non-overlapping prompt conditions, but in natural rhetorical use, figures routinely co-occur ("figural collocation"), and effects commonly ascribed to a single figure are often the combined effect of several figures converging in the same passage ("figural convergence"). Treating each figure as an isolated condition is a useful experimental simplification, not a reflection of how these figures actually interact in real text.
- **Small sample size.** Only 200 StrategyQA questions were evaluated. This was enough to surface directional trends (e.g. enumeratio's faithfulness gain, parison's steering result) but not enough to support tight confidence intervals or robust per-figure accuracy breakdowns.
- **Judge confounds in Part C.** The blind structure classification judge (Claude Haiku) is a weak, single-model signal that already struggles to separate parison from chiasmus at baseline (see Experiment A). When a steering setting fails to shift the judge's verdict, it's not possible to cleanly attribute that null result to (a) steering being ineffective, (b) the judge being too weak to detect a real structural shift, or (c) the underlying activation cluster being too entangled for a single direction vector to separate. These three explanations are observationally identical from the outside and were not disentangled here.
- **Single, small model.** All experiments used Llama-2-7b-chat-hf. A 7B model may simply lack the capacity to reliably execute more structurally demanding figures like chiasmus, and the probe/steering results may not transfer to larger or differently-trained models.
- **Narrow figure set.** Only three rhetorical figures (plus baseline CoT) were tested. The results say little about how other figures (e.g. syllogismus, anaphora, antithesis) would behave under the same pipeline.
- **CAA scaling factors were too large.** Every tested CAA strength caused 100% parse failures and degenerate single-token repetition (see Experiment C). CAA was therefore never evaluated in a regime where it could plausibly produce a meaningful causal effect; the negative result reflects miscalibrated magnitude, not necessarily a ceiling on what CAA could do with proper normalisation.
- **Steering direction applied uniformly across layers.** The reported steering results broadcast a single direction vector across the top-5 layers simultaneously, rather than testing steering on one layer at a time. This makes it harder to localise which specific layer drives the (modest) causal effect observed for parison.
- **Mismatch between figure and evaluation task.** StrategyQA tests factual binary reasoning — whether the model knows the right facts and can chain them to a yes/no conclusion. Enumeratio's communicative function (sequential decomposition, comprehensive coverage) is well matched to that demand, which is plausibly why it's the only figure that beats baseline on faithfulness. Parison and chiasmus are harder to map onto StrategyQA: parison's function (parallel emphasis across comparable elements) is better suited to comparative reasoning tasks, and chiasmus's (structural inversion/contrast) is better suited to contrastive or dialectical tasks. The weak results for parison and chiasmus may partly reflect this benchmark mismatch rather than a true failure of the figure to scaffold reasoning. More generally, the evaluation task should be chosen to match the communicative function of the figure under test.
...
- **Match evaluation task to figure:** Test parison on a comparative-reasoning or argument-quality benchmark, chiasmus on a contrastive/dialectical task, and enumeratio on stepwise decomposition tasks like StrategyQA, rather than evaluating all figures on a single factual-reasoning benchmark. A persuasiveness dataset may also better surface effects for figures whose communicative function is rhetorical impact rather than factual chaining.
